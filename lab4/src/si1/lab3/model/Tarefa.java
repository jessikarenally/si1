...
public class Tarefa implements Comparable<String> {
	...
	private boolean verificaValidadeData(Date data) {
		...
		if((dataAtual.getYear() + 1900) > data.getYear()){
			return false;
		}else if ((dataAtual.getYear() + 1900) == data.getYear()){
	...
	private boolean diaValido(Date data) {
		...
		if(data.getMonth() > 11 || data.getMonth() <0 ){ return false;
			...
			if((data.getYear() + 1900) % 400  == 0 || ((data.getYear() + 1900) % 4 == 0 && (data.getYear() + 1900) % 100  != 0 ))
		...
		if((data.getYear() +1900)  < 0) return false;
	...
	public String getDataFormatada() {
		if(date.equals(null)) return "00000000";
		dataFormatada = String.valueOf(date.getDate()) +  String.valueOf(date.getMonth()) + String.valueOf(date.getYear());
